...
import { createChat } from "./geminiClient";
...
export async function runAgentTurn(lineUserId: string, userText: string): Promise<string> {
  ...
  try {
    const chat = createChat(toolDeclarations, buildSystemPrompt(new Date()), history);
    ...
    finalText = result.text || "うまく応答できなかった…もう一回言ってみて？";
  } catch (err) {
